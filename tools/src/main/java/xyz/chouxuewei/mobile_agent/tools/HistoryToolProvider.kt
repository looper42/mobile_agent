...
class HistoryToolProvider(private val conversations: ConversationStore) : ToolProvider {
    ...
    override val definitions get() = listOf(
        ...
        ToolDefinition(
            ...
            inputSchema = localizedJsonSchema("""{"type":"object","properties":{"message_ids":{"type":"array","items":{"type":"string"},"minItems":1,"maxItems":10,"description":localizedText("history_search 返回的真实消息 ID", "Actual message ID returned by history_search")},"tool_call_id":{"type":"string","description":localizedText("长工具结果提示中返回的真实工具调用 ID", "Actual tool call ID returned in the long-result notice")},"offset":{"type":"integer","minimum":0,"default":0,"description":localizedText("仅用于分段读取工具结果", "Only for chunked tool-result reads")},"max_chars":{"type":"integer","minimum":1,"maximum":8000,"default":8000},"scope":{"type":"string","enum":["current","all"],"default":"current","description":localizedText("all 需要用户明确要求跨会话读取", "all requires an explicit request for cross-conversation reads")}},"additionalProperties":false}"""),
            sideEffect = ToolSideEffect.READ,
            providerId = "history",
