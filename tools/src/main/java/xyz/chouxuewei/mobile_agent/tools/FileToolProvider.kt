...
class FileToolProvider(
    ...
    override val title get() = localizedText("文件", "File")
    override val description get() = localizedText("读取你添加的文本文件，并保存 AI 生成的文件。", "Read text files you add and save files generated by AI.")

    override val definitions get() = listOf(
        ToolDefinition(
            "file_list", localizedText("查看对话文件", "View conversation files"),
            localizedText("列出当前会话中由用户添加的附件和 AI 生成的产物，不会遍历手机其他文件。不知道准确的 uri、artifact_id 或产物名称时先调用此工具。", "List user attachments and AI-generated artifacts in the current conversation without scanning other phone files. Call this first when the exact uri, artifact_id, or artifact name is unknown."),
            """{"type":"object","properties":{},"additionalProperties":false}""",
            ToolSideEffect.READ, "files",
            approvalDescription = localizedText("查看这段对话中的附件和已生成文件。", "View attachments and generated files in this conversation."),
        ),
        ToolDefinition(
        ...
        ToolDefinition(
            "file_write", localizedText("生成文本文件", "Generate text file"),
            localizedText("仅在用户需要文件或任务确实需要可保存产物时，在 App 工作区生成完整的 UTF-8 文本文件；不要写入占位内容。同名文件会自动使用新名称，不覆盖旧产物。文件名支持安全 Unicode，但不能包含路径分隔符或控制字符。", "Generate a complete UTF-8 text file in the app workspace only when the user needs a file or the task requires a saved artifact. Do not write placeholder content. Duplicate names receive a new name and never overwrite old artifacts. Safe Unicode is allowed, but path separators and control characters are not."),
            localizedJsonSchema("""{"type":"object","properties":{"name":{"type":"string","minLength":1,"maxLength":120,"description":localizedText("包含扩展名的文件名，不能包含 /、\\ 或控制字符", "Filename including extension; cannot contain /, \\, or control characters")},"content":{"type":"string","maxLength":500000,"description":localizedText("要保存的完整文件正文", "Complete file content to save")}},"required":["name","content"],"additionalProperties":false}"""),
            ToolSideEffect.LOCAL_WRITE, "files",
            approvalDescription = localizedText("在 Mobile Agent 中保存一个文本文件。", "Save a text file in Mobile Agent."),
        ),
        ToolDefinition(
    ...
    override fun approvalSummary(call: RequestedToolCall): String? = runCatching {
        ...
        when (call.toolId) {
            "file_write" -> args["name"]?.jsonPrimitive?.contentOrNull
                ?.trim()?.take(120)?.let { localizedText("文件名：$it", "Filename: $it") }
    ...
    private suspend fun list(context: ToolExecutionContext): ToolResult {
        ...
        return ToolResult(content, localizedText("找到 ${refs.size} 个附件和 ${generated.size} 个已生成文件", "Found ${refs.size} attachments and ${generated.size} generated files"))
    }
    ...
    private suspend fun read(args: JsonObject, context: ToolExecutionContext): ToolResult {
        ...
        return ToolResult(result, localizedText("已读取 ${source.name} 的 ${chunk.text.length} 个字符", "Read ${chunk.text.length} characters from ${source.name}"))
    }
    ...
    private suspend fun write(args: JsonObject, context: ToolExecutionContext): ToolResult {
        ...
        val content = args["content"]?.jsonPrimitive?.content ?: error(localizedText("缺少文件内容", "File content is missing."))
        require(content.length <= 500_000) { localizedText("单个文本文件不能超过 500000 个字符", "A text file cannot exceed 500000 characters.") }
    ...
    private suspend fun share(args: JsonObject, context: ToolExecutionContext): ToolResult {
        ...
        require(listOf(artifactId, uriValue).count { !it.isNullOrBlank() } == 1) {
            localizedText("artifact_id 和 uri 必须且只能提供一个", "Provide exactly one of artifact_id or uri.")
        }
        ...
        check(appContext.packageManager.resolveActivity(send, 0) != null) { localizedText("没有应用可以接收这个文件", "No app can receive this file.") }
        val title = args["title"]?.jsonPrimitive?.contentOrNull?.take(200).orEmpty().ifBlank { localizedText("分享 $name", "Share $name") }
