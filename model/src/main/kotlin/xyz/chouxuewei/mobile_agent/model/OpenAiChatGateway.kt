    ...
    override fun stream(request: ChatRequest): Flow<ModelEvent> = callbackFlow {
        ...
        val payload = buildJsonObject {
                ...
                request.tools.forEach { tool ->
                    add(buildJsonObject {
                        ...
                        putJsonObject("function") {
                            ...
                            put("parameters", runCatching { Json.parseToJsonElement(tool.inputSchema) }
            ...
            request.reasoningEffort?.takeIf(String::isNotBlank)?.let { effort ->
                config.reasoningEffortField.takeIf(String::isNotBlank)?.let { field -> put(field, effort) }
            }
                ...
                request.messages.forEach { turn ->
                    add(buildJsonObject {
                        ...
                        if (turn.role == "assistant" && turn.toolCalls.isNotEmpty()) {
                            if (turn.content.isEmpty()) put("content", JsonNull) else put("content", turn.content)
                            putJsonArray("tool_calls") {
                                turn.toolCalls.forEach { call -> add(buildJsonObject {
                                    put("id", call.id); put("type", "function")
                                    putJsonObject("function") {
                                        put("name", call.toolId); put("arguments", call.argumentsJson)
                                    }
                                }) }
                            }
                        } else {
                                ...
                                putJsonArray("content") {
                                    ...
                                    turn.images.forEach { image -> add(buildJsonObject {
                                        ...
                                        putJsonObject("image_url") {
                                            put("url", "data:${image.mimeType};base64,${image.bytes.toByteString().base64()}")
                                            put("detail", "auto")
                                        }
                                    }) }
                                }
        ...
        AgentLog.i("Model") {
            "request_start model=$model messages=${request.messages.size} tools=${request.tools.size} images=${request.messages.sumOf { it.images.size }} payload_bytes=${payloadText.toByteArray().size} max_output=${request.maxOutputTokens}"
        }
        val call = client.newCall(Request.Builder().url("$base/chat/completions")
            .header("Authorization", "Bearer ${config.apiKey}")
            .header("Accept", "text/event-stream")
            .post(payloadText.toRequestBody("application/json; charset=utf-8".toMediaType())).build())
                ...
                launch(Dispatchers.IO) {
                    try {
                        response.use {
                            ...
                            data class PendingCall(var id: String = "", var name: String = "", val arguments: StringBuilder = StringBuilder())
                            ...
                            suspend fun dispatch() {
                                ...
                                json["usage"]?.takeUnless { u -> u is JsonNull }?.jsonObject?.let { usage ->
                                    send(ModelEvent.Usage(usage["prompt_tokens"]?.jsonPrimitive?.intOrNull, usage["completion_tokens"]?.jsonPrimitive?.intOrNull))
                                }
                                val choice = json["choices"]?.jsonArray?.firstOrNull()?.jsonObject ?: return
                                val delta = choice["delta"]?.jsonObject
                                delta?.get("tool_calls")?.jsonArray?.forEach { element ->
                                    val value = element.jsonObject
                                    val index = value["index"]?.jsonPrimitive?.intOrNull ?: pendingCalls.size
                                    val pending = pendingCalls.getOrPut(index) { PendingCall() }
                                    value["id"]?.jsonPrimitive?.contentOrNull?.let { pending.id = it }
                                    value["function"]?.jsonObject?.let { function ->
                                        ...
                                        function["arguments"]?.jsonPrimitive?.contentOrNull?.let(pending.arguments::append)
                                    }
                                }
                                listOf("reasoning_content", "reasoning", "thinking").firstNotNullOfOrNull { key ->
                                    (delta?.get(key) as? JsonPrimitive)?.contentOrNull?.takeIf(String::isNotEmpty)
                                }?.let { text -> send(ModelEvent.ReasoningDelta(text)) }
                                (delta?.get("content") as? JsonPrimitive)?.contentOrNull
                                    ?.takeIf(String::isNotEmpty)?.let { text -> send(ModelEvent.TextDelta(text)) }
                            ...
                            pendingCalls.values.forEach { pending ->
                                if (pending.id.isNotBlank() && pending.name.isNotBlank()) {
                                    send(ModelEvent.ToolCall(RequestedToolCall(
                                        pending.id, pending.name, pending.arguments.toString().ifBlank { "{}" },
                                    )))
                                }
                            }
                            AgentLog.i("Model") {
